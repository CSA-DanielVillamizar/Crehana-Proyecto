...
$webappname="creahana-webapp$(Get-Random)"
$location="east us"

# Crear un grupo de recursos.
New-AzResourceGroup -Name RG-Apps-Prod -Location $location

# Cree un plan de App Service en el nivel Gratis.
New-AzAppServicePlan -Name $webappname -Location $location -ResourceGroupName RG-Apps-Prod -Tier Free

# Cree una aplicación web.
New-AzWebApp -Name $webappname -Location $location -AppServicePlan $webappname -ResourceGroupName RG-Apps-Prod

# Configure la implementación de GitHub desde el repositorio de GitHub e in deploy una vez.
$PropertiesObject = @{
    repoUrl = "$gitrepo";
    branch = "master";
    isManualIntegration = "true";
}
Set-AzResource -Properties $PropertiesObject -ResourceGroupName RG-Apps-Prod -ResourceType Microsoft.Web/sites/sourcecontrols -ResourceName $webappname/web -ApiVersion 2015-08-01 -Force

# Eliminar recursos creados
Remove-AzResourceGroup -Name RG-Apps-Prod -Force